-- ============================================================
-- Kerala Family Cookbook — Stage 2: the save_recipe function
-- Run this ONCE in the Supabase SQL Editor.
-- Safe to re-run (create or replace). It adds ONE function and
-- changes no tables and no data.
-- ============================================================
--
-- save_recipe(payload) writes an entire recipe — its top-level
-- fields, all sections, all ingredients, and all steps — in a
-- SINGLE transaction. If any part fails, the whole save rolls
-- back, so a recipe can never be left half-updated.
--
-- Sections, ingredients, and steps are all updated in place by
-- their id, so any external reference to an id (and a step's
-- own photos, which FK to recipe_steps.id) survives a save. A
-- row is only deleted when the editor removes it; a row with a
-- NULL id is treated as new and inserted.
--
-- An ingredient that belongs to a newly added (un-saved) section
-- uses section_client_key — a string the editor generates — to
-- point at the matching new section in the payload. The function
-- maps each client_key to its new id and resolves the ingredient.
-- ============================================================
...
begin
  ...
  --    references one of these — which would mean the editor has
  --    a bug; we want to know about it loudly.
  -- ------------------------------------------------------------
  ...
  -- ------------------------------------------------------------
  -- 7. Steps — updated in place by id so step photos survive.
  -- ------------------------------------------------------------
